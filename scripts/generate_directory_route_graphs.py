...
def add_edge(edges: list[dict], edge_keys: set[tuple[str, str]], floor_id: str, from_node: dict, to_node: dict, source: str, distance_override: float | None = None) -> None:
    key = tuple(sorted((from_node["id"], to_node["id"])))
    if from_node["id"] == to_node["id"] or key in edge_keys:
        return
    edge_keys.add(key)
    edges.append({
        ...
        "toNodeId": to_node["id"],
        "distance": round(distance_override if distance_override is not None else math.hypot(from_node["x"] - to_node["x"], from_node["y"] - to_node["y"]), 2),
        "accessible": True,
        "source": source,
    })
...
def graph_for_floor(floor: dict) -> dict:
    ...
    for (x, y), node in grid.items():
        for dx, dy in ((GRID_STEP, 0), (0, GRID_STEP), (GRID_STEP, GRID_STEP), (-GRID_STEP, GRID_STEP)):
            ...
            if not other or not line_is_walkable(interior, (x, y), (x + dx, y + dy)):
                continue
            add_edge(edges, edge_keys, floor_id, node, other, "directory-map-black-circulation")

    max_offset = int(SHORTCUT_MAX_DISTANCE // GRID_STEP)
    offsets = []
    for oy in range(-max_offset, max_offset + 1):
        for ox in range(-max_offset, max_offset + 1):
            ...
            if ox == 0 and oy == 0:
                continue
            distance = math.hypot(ox * GRID_STEP, oy * GRID_STEP)
            if distance <= GRID_STEP * 1.5 or distance > SHORTCUT_MAX_DISTANCE:
                continue
            offsets.append((ox * GRID_STEP, oy * GRID_STEP, distance))
    for (x, y), node in grid.items():
        for dx, dy, edge_distance in offsets:
            other = grid.get((x + dx, y + dy))
            if other and line_is_walkable(interior, (x, y), (x + dx, y + dy)):
                add_edge(edges, edge_keys, floor_id, node, other, "directory-map-black-circulation-shortcut", edge_distance)
